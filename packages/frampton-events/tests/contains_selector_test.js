...
QUnit.test('returns true if event target has given selector', function(assert) {
  this.div1.classList.add('blue');
  assert.ok(containsSelector('.blue', { target: this.div1 }));
});

QUnit.test('returns true if event target contains given selector', function(assert) {
  this.div2.classList.add('blue');
  assert.ok(containsSelector('.blue', { target: this.div1 }));
});

QUnit.test('returns false if event target does not contain given selector', function(assert) {
  this.div2.classList.add('blue');
